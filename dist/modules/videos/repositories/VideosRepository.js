...
class VideoRepository {
    ...
    getVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM videos WHERE user_id = ?', [user_id], (error, results, fileds) => {
                ...
                return response.status(400).json({ message: "Videos retornados com sucesso", videos: results });
            });
        });
    }
    searchVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query('SELECT * FROM videos WHERE description or title LIKE ?', [`%${search}%`], (error, results, fileds) => {
                connection.release();
                if (error) {
                    return response.status(400).json({ error: "erro ao buscar os videos!" });
                }
